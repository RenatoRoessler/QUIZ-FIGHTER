...
export class LoginComponent implements OnInit {
  ...
  ngOnInit() {
    // Se identificar que o usuario está logado redireciona para a tela de pré jogo
  ...
  logarGoogle() {
    this.afAuth.auth.signInWithPopup(
      ...
      .catch(err => this.snackBar.open(
              'Problema ao autenticar no Google.', 'Erro', { duration: 5000 })
    );
  }
  ...
  logarEmail() {
    ...
    this.afAuth.auth
      ...
      .then(res  => this.router.navigate([this.PRE_JOGO_PATH]))
      .catch(err => this.snackBar.open(
              'Usuário/senha inválido(s)', 'Erro', { duration: 5000 })
    );
  }
  ...
  cadastrarEmail() {
    ...
    this.afAuth.auth.createUserAndRetrieveDataWithEmailAndPassword(
      ...
      .then(res  => this.router.navigate([this.PRE_JOGO_PATH]))
      .catch(err => this.snackBar.open(
              'Problema ao cadastrar email.', 'Erro', { duration: 5000 })
    );
  }
