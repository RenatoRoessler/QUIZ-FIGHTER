...
export class JogoService {
  ...
  adicionarJogoPadrao(qtdJogos: number) {
    if (qtdJogos <= 0 ) {
      return;
    }
    ...
    this.snackBar.open(
      'Jogos incializados com sucesso',
      'OK', this.SNACKBAR_DURATION);
  }

  async removerTodosJogos(): Promise<void> {
    const jogos: firebase.firestore.QuerySnapshot = await this.afs.collection(this.JOGOS_DOC_PATH).ref.get();
    const batch = this.afs.firestore.batch();
    jogos.forEach(jogo => batch.delete(jogo.ref));
    return batch.commit();
  }

  incializarJogos(data: any) {
    if (data.manterJogosExistentes) {
      this.adicionarJogoPadrao(data.qtdJogos);
    } else {
      this.removerTodosJogos()
        .then(res => this.adicionarJogoPadrao(data.qtdJogos))
